...
function main() {
  ...
  scene.onLoad.add(() => {
    ...
    const gameLayer = new g.E({
      scene
    });
    const scoreLayer = new g.E({
      scene
    });
    const resultLayer = new g.E({
      scene
    });
    ...
    // ボタン作成
    const createButton = (text, y, color, onClick, strokeColor) => {
      const width = 200;
      const height = 60;
      const fontSize = 30;
      ...
      btn.append(new g.FilledRect({
        scene,
        cssColor: strokeColor || "transparent",
        width,
        height
      }));
      btn.append(new g.FilledRect({
    ...
    const registerPlayer = pid => {
      ...
      const label = new g.Label({
        ...
        text: `Player ${pid}: 0`,
        font,
        textColor: pid === g.game.selfId ? "green" : "black",
        // 自分は緑
        x: 10,
    ...
    const showTitle = () => {
      var _a;
      gameState = "title";
      (_a = resultLayer.children) === null || _a === void 0 ? void 0 : _a.slice().forEach(c => c.destroy());
    ...
    const finishGame = () => {
      ...
      (_a = gameLayer.children) === null || _a === void 0 ? void 0 : _a.slice().forEach(c => c.destroy());
      // 結果発表時の背景
      ...
      const resultTitle = new g.Label({
        ...
        fontSize: 40,
        textColor: "white",
        x: (g.game.width - 240) / 2,
        // 中央寄せ
        y: 50
      });
      resultLayer.append(resultTitle);
      const ranking = Object.keys(scores).map(pid => ({
        id: pid,
        score: scores[pid]
    ...
    scene.onMessage.add(ev => {
      ...
      if (ev.data.type === "req_start") {
        ...
        if (target !== undefined && target.destroyed() === false) {
          ...
          scene.setTimeout(() => {
            if (popup.destroyed() === false) popup.destroy();
          }, 500);
        }
      }
    });
